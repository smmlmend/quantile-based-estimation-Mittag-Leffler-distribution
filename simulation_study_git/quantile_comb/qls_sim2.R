...
data <- list()
res_qls2 <- data.frame(matrix(nrow =  1000, ncol = 5))
res_qls2[,1] <- rep(combs[k,1], each = 1000)
res_qls2[,2] <- rep(combs[k,2], each = 1000)
res_qls2[,3] <- rep(combs[k,3], each = 1000)
...
for (j in 1:1000) {
  ...
  res_qls2[j,4:5] <- mlqls(data[[j]], q_comb[[2]])$par
}


# Save the object to the file
filename <- paste("res_qls1_setting_", k ,".RData", sep = "")
save(res_qls2, file = filename)