...
q_comb <- list(c(0.05, 0.2, 0.475, 0.8, 0.925),
               ...
               c(0.1, 0.3, 0.5, 0.8, 0.925))


# simulation study --------------------------------------------------------

data <- list()
res_qls4 <- data.frame(matrix(nrow =  1000, ncol = 5))
res_qls4[,1] <- rep(combs[k,1], each = 1000)
res_qls4[,2] <- rep(combs[k,2], each = 1000)
res_qls4[,3] <- rep(combs[k,3], each = 1000)
...
for (j in 1:1000) {
  ...
  res_qls4[j,4:5] <- mlqls(data[[j]], q_comb[[4]])$par
}


# Save the object to the file
filename <- paste("res_qls4_setting_", k ,".RData", sep = "")
save(res_qls4, file = filename)