...
data <- list()
res_qls <- data.frame(matrix(nrow = 288 * 1000, ncol = 5))
res_qls[,1] <- rep(combs[,1], each = 1000)
res_qls[,2] <- rep(combs[,2], each = 1000)
res_qls[,3] <- rep(combs[,3], each = 1000)
...
for(k in 1:288){
  ...
  for (j in 1:1000) {
    ...
    res_qls[(k-1)*1000+j,4:5] <- mlqls(data[[j]], 
                                       q_comb[[1]])$par
  }
}

# Save the object to the file
filename <- paste("res_qls_", k ,".RData", sep = "")
save(res_qls, file = filename)