...
moving_mle <- function(data, times, k, type, q = c(0.1, 0.23, 0.45,
                                                   0.7, 0.93) ){
  year(times) <- 2002
  days <- as.POSIXct(
  ...
  if (type == "ml") {
    for (i in 1:365) {
      ...
      if(length(use) < 2){
        ...
        res[i, 2:3] <- weighted.mlmle(data[[1]][use], w)$par
      }
    }
  }
  if(type == "qb"){
    for (i in 1:365) {
      ...
      if(length(use) < 2){
        ...
        res[i, 2:3] <- weighted.mlqbe(data[[1]][use], w, q = q)$par
      }
    }
  }
  return(res)
...
distance <- matrix(nrow = 10000, ncol = 2)
...
for (i in 1:10000) {
  ...
  distance[i,] <-  colMeans((
    moving_mle(unlist(out_6h[j,]$WW)[ind], time[[j]][ind], 45, "logmom")[,2:3] -
      moving_mle(unlist(out_6h[j,]$WW)[ind2], time[[j]][ind2], 45, "logmom")[,2:3])^2,
    na.rm = TRUE)
}
...
## beta
filename <- paste("perm_test_1940_", j ,".RData", sep = "")
